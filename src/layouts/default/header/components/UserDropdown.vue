<script lang="tsx">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      console.log(userStore.getUserInfo.userName, 'leven');
      const getUserInfo = computed(() => {
        const { userName = '', headerImg, desc, ID } = userStore.getUserInfo || {};
        console.log(headerImg, ID);
        return { userName, avatar: headerImg || headerImg, desc };
      });
        ...
        return (
          ...
          >
            <div class="app-layout-header-anction-icon">
              ...
              <span class="text-14px text-$text-color">{getUserInfo.value.userName}</span>
            </div>
          </NDropdown>
        );
      };
